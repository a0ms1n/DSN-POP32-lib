...
Psx::Psx(byte dataPin, byte cmndPin, byte attPin, byte clockPin, byte delay):
...
void Psx::setupPins(byte dataPin, byte cmndPin, byte attPin, byte clockPin, byte delay) {
	...
	// Some typo notice at attPin, clockPin in original source code.
...
unsigned int Psx::read() {
	...
	AnLx = shift(0xFF);
	AnLy = shift(0xFF);
	if((AnRx==255)&&(AnRy==255)&&(AnLx==255)&&(AnLy==255)){
		AnRx = 127;
		AnRy = 127;
		AnLx = 127;
		AnLy = 127;
