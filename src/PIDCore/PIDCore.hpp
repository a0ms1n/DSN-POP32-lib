...
inline PIDCore::PIDCore(const PIDGains &gains, const double_t &outMin, const double_t &outMax,const double_t IErrorTreshold = 0)
...
bool PIDCore::Compute()
{
    if(dtTimer.check()){
        ...
        if(abs(iTerm) <= gains.IErrorTreshold)iTerm = 0;
        ...
        output_sum += iTerm;
        if(abs(iTerm) <= gains.IErrorTreshold)output_sum = 0;
...
bool PIDCore::Compute(double_t _error)
{
    if(dtTimer.check()){
        double_t _input = *current;
        double_t dInput = _input - last_input;  
        switch(errormode){
            case errorMode::errorLinear:
                error = (_error)*gains.Knorm;
                break;
            case errorMode::errorAbsolute:
                error = abs(_error*gains.Knorm);
                break;
            case errorMode::errorSquare:
                error = (_error)*gains.Knorm;
                error = (error<0)?(error)*(-error):error*error;
